...
from .severity import update_severity
...
class OptionsMenu:
    ...
    def __init__(self, client: SnykClient):
        self.client = client
        self.fix_manager = FixPRManager(client.token)
    ...
    def show(self, org: dict, results: list[dict]) -> str:
        ...
        while True:
            ...
            console.print("  [cyan]2[/cyan] - Generate report (Markdown + CSV)")
            console.print("  [cyan]3[/cyan] - Trigger fix PRs")
            console.print("  [cyan]4[/cyan] - Update severity (based on risk score)")
            console.print("  [cyan]5[/cyan] - Manage .snyk ignores")
            console.print("  [cyan]6[/cyan] - Rescan org")
            ...
            if choice == "1":
                ...
                self.fix_manager.handle_fix_prs(org["id"], org.get("slug", org["id"]), results)

            elif choice == "4":
                update_severity(self.client, org["id"], results)

            elif choice == "5":
                manage_ignores(results)
